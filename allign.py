...
def proccess_image(image):
    ...
    with mp_face_mesh.FaceMesh(
        ...
        right_eye = (
            sum(x) / len(right_eye_landmarks),
            sum(y) / len(right_eye_landmarks),
        )
        ...
        image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # bugged once
...
def main(
    ...
    processed_image_directory=PROCESSED_IMAGE_DIRECTORY,
    image_name=None,
):
...
assert not (
    "--one-image" in args and "--from-directory" in args
), "Cannot use one-image and from-directory modes at the same time"

if "--cli" in args:
    assert (
        "--one-image" in args or "--from-directory" in args
    ), "CLI mode must be used with either --one-image or --from-directory."

    src_directory = input(
        f"Enter source image directory excluding filename(If empty {DATASET_DIRECTORY} will be used): "
    )
    if not src_directory:
        src_directory = DATASET_DIRECTORY

    out_directory = input(
        f"Enter output directory excluding file name (if empty {PROCESSED_IMAGE_DIRECTORY} will be used): "
    )
    if not out_directory:
        out_directory = PROCESSED_IMAGE_DIRECTORY
    ...
    if "--one-image" in args:
        ...
        main(
            single_image=True,
            dataset_directory=src_directory,
            processed_image_directory=out_directory,
            image_name=name,
        )
